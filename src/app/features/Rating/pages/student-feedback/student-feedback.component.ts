...
export class StudentFeedbackComponent {
  ...
  getValidationMessage(control: any): string {
    ...
    if (control.errors['min']) return 'الرجاء اختيار تقييم';
  ...
  onSubmit(): void {
    ...
    this.ratingService.SubmitRating(ratingRequest).subscribe({
      next: () => {
        ...
        this.toastService.show('شكراً لك، تم إرسال تقييمك بنجاح وحصلت على 10 نقاط.');
      ...
      error: (err) => {
        ...
        this.toastService.show('حدث خطأ، حاول مرة أخرى', 'error');
      },
    });
  }
}
