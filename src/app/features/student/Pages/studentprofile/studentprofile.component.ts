...
export class StudentprofileComponent
    ...
    this.rewardService
      ...
      .subscribe({
        ...
        next: (response) => {
          ...
          this.poService
            .AddPointsToUser(response.pointsAdded, userId)
            .subscribe({
  ...
  knowledgePoints = computed(() =>
    this.student()?.knowledgePoints ?? 0
  );


  knowledgeProgress = computed(() =>
    this.student()?.knowledgeProgress ?? 0
  );


  streak = computed(() =>
    this.student()?.streakCount ?? 0
  );
  ...
  private startAutoScroll(): void {
    ...
    const animate = () => {
      ...
      const slider =
        this.coursesSlider?.nativeElement;


      if (
        slider &&
        this.recommendedCourses().length > 0
      ) {
