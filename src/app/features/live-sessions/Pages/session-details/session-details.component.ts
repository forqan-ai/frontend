...
import {
  ILiveSession,
  SessionStatus,
} from '../../Models/live-session.interface';
import {
  IBooking,
  BookingStatus,
...
export class SessionDetailsComponent implements OnInit {
        ...
        this.learningCirclesService.getDetails(res.circleId).subscribe({
          next: (circle) => {
            this.canManageSessions.set(
              circle.permissions.canCreateLiveSession
            );

            if (this.canManageSessions()) {
              this.bookingsService
                .getSessionBookings(this.sessionId)
                .subscribe({
                  next: (bookings) =>
                    this.attendees.set(bookings),
                  error: console.error,
                });
            }
          ...
          error: (err) => {
            ...
            this.canManageSessions.set(
              this.authService.getRole() === Role.Teacher &&
              this.authService.getUserId() === res.teacherId
            );
  ...
  openZoomSession(): void {
    ...
    const url = this.canManageSessions()
      ? s.zoomStartUrl
      : s.zoomJoinUrl;
    ...
    if (!url) {
      this.confirmDialog.set({
        isOpen: true,
        title: 'رابط الجلسة غير متوفر',
        message: 'لم يتم إنشاء رابط Zoom لهذه الجلسة بعد.',
        confirmText: 'حسناً',
        type: 'primary',
  ...
  cancelBooking(): void {
    ...
    this.confirmDialog.set({
      ...
      title: 'إلغاء الحجز',
      message:
        'هل أنت متأكد من رغبتك في إلغاء حجزك لهذه الجلسة؟',
  ...
  cancelSession(): void {
    this.confirmDialog.set({
      ...
      title: 'إلغاء الجلسة',
      message:
        'هل أنت متأكد من إلغاء هذه الجلسة؟ لا يمكن التراجع عن هذا الإجراء.',
  ...
  confirmAttendee(bookingId: string): void {
    this.confirmDialog.set({
      ...
      title: 'تأكيد الحضور',
      message:
        'هل أنت متأكد من حضور هذا الطالب للجلسة؟',
  ...
  completeAttendee(bookingId: string): void {
    this.confirmDialog.set({
      ...
      message:
        'هل تريد إكمال الجلسة لهذا الطالب وإضافة النقاط له؟',
