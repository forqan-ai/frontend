...
interface DetailsError { title: string; message: string; retryable: boolean; }
type Submission = 'proposal' | 'meeting' | TeacherConsultationAction;

@Component({
  selector: 'app-teacher-consultation-details',
  imports: [RouterLink, ConsultationProposalFormComponent, MeetingLinkFormComponent, TeacherConsultationActionModalComponent],
...
export class TeacherConsultationDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.consultationId) {
      this.loadError.set({ title: 'تعذر فتح الاستشارة', message: 'معرّف الاستشارة غير صالح.', retryable: false });
      return;
    }
  ...
  loadConsultation(preserveActionError = false): void {
    ...
    if (!preserveActionError) this.actionError.set('');
  ...
  saveProposal(request: ConsultationProposalRequest): void {
    ...
    if (!details?.actions.canEditProposal || this.submitting()) return;
    const successMessage = details.status === 'Requested'
      ? 'تم إرسال عرض الاستشارة إلى الطالب بنجاح.'
      : 'تم حفظ تعديلات عرض الاستشارة بنجاح.';
    this.runMutation('proposal', this.consultationService.updateProposal(this.consultationId, request), successMessage);
  }

  saveMeetingLink(request: SetMeetingLinkRequest): void {
    const details = this.consultation();
    if (!details?.actions.canSetMeetingLink || this.submitting()) return;
    this.runMutation('meeting', this.consultationService.setMeetingLink(this.consultationId, request), 'تم حفظ رابط الاستشارة بنجاح.');
  }

  openAction(action: TeacherConsultationAction): void {
    const actions = this.consultation()?.actions;
    const allowed = action === 'reject' ? actions?.canReject : action === 'cancel' ? actions?.canCancel : actions?.canComplete;
    if (allowed && !this.submitting()) this.actionModal.set(action);
  }

  closeAction(): void {
    if (!this.submitting()) this.actionModal.set(null);
  }

  confirmAction(event: { action: TeacherConsultationAction; reason: string | null }): void {
    const details = this.consultation();
    if (!details || this.submitting()) return;

    let request$: Observable<ConsultationDetails>;
    let successMessage: string;
    if (event.action === 'reject' && details.actions.canReject) {
      request$ = this.consultationService.rejectConsultation(this.consultationId, { reason: event.reason });
    ...
    this.runMutation(event.action, request$, successMessage);
  }

  private runMutation(type: Submission, request$: Observable<ConsultationDetails>, successMessage: string): void {
  ...
  private handleMutationError(error: HttpErrorResponse, type: Submission): void {
    ...
    if (error.status === 403) message = 'لم يعد مسموحًا بتنفيذ هذا الإجراء على الاستشارة.';
    else if (error.status === 404) message = 'تعذر العثور على الاستشارة المطلوبة.';
    else if (error.status === 400 && failure?.errorMessage) message = failure.errorMessage;
    else message = 'تعذر تنفيذ الإجراء. تحقق من البيانات وحاول مرة أخرى.';

    this.actionModal.set(null);
    this.actionError.set(message);
    this.toast.show(message, 'error');
    if (error.status === 403 || error.status === 404) this.loadConsultation(true);
  }

  private detailsErrorFor(error: HttpErrorResponse): DetailsError {
    if (error.status === 403) return { title: 'لا يمكنك الوصول إلى هذه الاستشارة', message: 'هذه الاستشارة غير متاحة لهذا الحساب.', retryable: false };
    if (error.status === 404) return { title: 'الاستشارة غير موجودة', message: 'تعذر العثور على الاستشارة المطلوبة أو لم تعد متاحة.', retryable: false };
    return { title: 'تعذر تحميل تفاصيل الاستشارة', message: 'حدث خطأ أثناء الاتصال. تحقق من اتصالك ثم حاول مرة أخرى.', retryable: true };
  }
}
